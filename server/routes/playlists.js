import { Router } from 'express';
import mongoose from 'mongoose';
import Playlist from '../models/Playlist.js';
import Track from '../models/Track.js';

const router = Router();

// GET /api/playlists
router.get('/playlists', async (req, res) => {
  const playlists = await Playlist.find({ type: 'manual' }).sort({ createdAt: -1 }).lean();

  // Collect up to 4 unique cover hashes per playlist in one batch query
  const allIds = playlists.flatMap(p => p.trackIds.slice(0, 40));
  const tracks = allIds.length
    ? await Track.find({ _id: { $in: allIds } }, { _id: 1, cover: 1 }).lean()
    : [];
  const coverByTrack = Object.fromEntries(tracks.map(t => [String(t._id), t.cover || null]));

  res.json(playlists.map(p => {
    const covers = [];
    for (const id of p.trackIds) {
      const c = coverByTrack[String(id)];
      if (c && !covers.includes(c)) covers.push(c);
      if (covers.length === 9) break;
    }
    return { _id: p._id, name: p.name, trackCount: p.trackIds.length, covers };
  }));
});

// POST /api/playlists
router.post('/playlists', async (req, res) => {
  const { name } = req.body;
  if (!name || typeof name !== 'string' || !name.trim()) {
    return res.status(400).json({ error: 'Name is required' });
  }
  const playlist = await Playlist.create({ name: name.trim(), type: 'manual', trackIds: [] });
  res.status(201).json(playlist);
});

// GET /api/playlists/:id
router.get('/playlists/:id', async (req, res) => {
  if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
    return res.status(404).json({ error: 'Not found' });
  }
  const playlist = await Playlist.findOne({ _id: req.params.id, type: 'manual' }).lean();
  if (!playlist) return res.status(404).json({ error: 'Not found' });

  const ordered = await Track.find({ _id: { $in: playlist.trackIds } }).lean();
  const map = Object.fromEntries(ordered.map(t => [String(t._id), t]));
  const tracks = playlist.trackIds.map(id => map[String(id)]).filter(Boolean);

  res.json({ ...playlist, tracks });
});

// PATCH /api/playlists/:id
router.patch('/playlists/:id', async (req, res) => {
  if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
    return res.status(404).json({ error: 'Not found' });
  }
  const playlist = await Playlist.findOne({ _id: req.params.id, type: 'manual' });
  if (!playlist) return res.status(404).json({ error: 'Not found' });

  const { name, trackIds } = req.body;
  if (name !== undefined) {
    if (typeof name !== 'string' || !name.trim()) return res.status(400).json({ error: 'Invalid name' });
    playlist.name = name.trim();
  }
  if (trackIds !== undefined) {
    if (!Array.isArray(trackIds)) return res.status(400).json({ error: 'Invalid trackIds' });
    playlist.trackIds = trackIds;
  }

  await playlist.save();
  res.json(playlist);
});

// DELETE /api/playlists/:id
router.delete('/playlists/:id', async (req, res) => {
  if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
    return res.status(404).json({ error: 'Not found' });
  }
  const playlist = await Playlist.findOneAndDelete({ _id: req.params.id, type: 'manual' });
  if (!playlist) return res.status(404).json({ error: 'Not found' });
  res.json({ ok: true });
});

// POST /api/playlists/:id/tracks
router.post('/playlists/:id/tracks', async (req, res) => {
  if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
    return res.status(404).json({ error: 'Not found' });
  }
  const playlist = await Playlist.findOne({ _id: req.params.id, type: 'manual' });
  if (!playlist) return res.status(404).json({ error: 'Not found' });

  const { trackIds } = req.body;
  if (!Array.isArray(trackIds) || !trackIds.length) return res.status(400).json({ error: 'trackIds required' });

  const existing = new Set(playlist.trackIds.map(String));
  for (const id of trackIds) {
    if (mongoose.Types.ObjectId.isValid(id) && !existing.has(String(id))) {
      playlist.trackIds.push(id);
      existing.add(String(id));
    }
  }
  await playlist.save();
  res.json(playlist);
});

// DELETE /api/playlists/:id/tracks/:trackId
router.delete('/playlists/:id/tracks/:trackId', async (req, res) => {
  if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
    return res.status(404).json({ error: 'Not found' });
  }
  const playlist = await Playlist.findOne({ _id: req.params.id, type: 'manual' });
  if (!playlist) return res.status(404).json({ error: 'Not found' });

  playlist.trackIds = playlist.trackIds.filter(id => String(id) !== req.params.trackId);
  await playlist.save();
  res.json(playlist);
});

export default router;
